  ...
  const handleSubmit = useCallback(async (data) => {
    try {
      ...
      const res = await AuthApi.resetPassword({
        tokenResetPassword: data.access_token,
        password: data.new_password,
      });
      toastSuccess("Đặt lại mật khẩu thành công");
      router.push("/login", { scroll: true });
    } catch (error) {
      toastError("Mã token sai, vui lòng nhập lại");
      console.log(error);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-1/2 h-full flex flex-col p-20 items-center gap-7  z-10">
        ...
        >
          <div className="w-full flex flex-col mb-2">
            <h3 className="text-3xl font-semibold mb-2">Đặt lại mật khẩu</h3>
            <div className="flex flex-row gap-1 ">
              <p className="text-base mb-2">
                Nhập token đã cấp và mật khẩu mới cho tài khoản
              </p>
            </div>
          </div>
          <div className="w-full flex flex-col">
            <div>
              <input
                type="text"
                placeholder="Token đã cấp"
                name="access_token"
                value={formik.values.access_token}
                onBlur={formik.handleBlur("access_token")}
